...
{
    public class AppUsedByDayDataAccess : DataAccess<AppUsedByDay, long>
    {
        public override int AddItem(AppUsedByDay item)
        {
            if (item == null)
            {
                throw new ArgumentException("AppUsedByDay");
            }

            using (SqlCommand command = new SqlCommand())
            {
                string query = @"
                        INSERT INTO [dbo].[AppUsedByDay]
                                   ([QueryData]
                                   ,[AppID]
                                   ,[Corpid]
                                   ,[UseCount]
        ...
        public override AppUsedByDay GetItem(long id)
        {
            return this.TargetSqlSession.QueryItems<AppUsedByDay>(u => u.ID == id).FirstOrDefault();
        }

        public override int UpdateItem(AppUsedByDay item)
        {
            if (item == null)
            {
                throw new ArgumentException("AppUsedByDay");
